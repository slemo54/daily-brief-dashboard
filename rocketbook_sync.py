        ...
        if match:
            try:
                return datetime.strptime(
                    match.group(1) if match.groups() else match.group(0), "%d %b %Y")
            except BaseException:
                pass
...
def generate_rocketbook_html(notes):
    ...
    for note in notes[:6]:  # Max 6 note visibili
        ...
        items.append(
            f'''                <a href="{pdf_url}" target="_blank" class="rocketbook-item">
...
def push_to_github():
    ...
    subprocess.run(["git", "config", "user.email",
                   "bot@dailybrief.local"], check=False)
    subprocess.run(["git", "config", "user.name",
                   "Daily Brief Bot"], check=False)
