import { sendYouTubeDigestEmail, YouTubeDigestData } from '../src/lib/email';
import * as fs from 'fs';
import * as path from 'path';

/**
 * Script per inviare l'email del YouTube AI Digest
 * Usage: ts-node scripts/send-youtube-digest.ts [path-to-markdown-file]
 */
...
async function main() {
  ...
  // Parse video data from markdown (simplified parsing)
  const videos: YouTubeDigestData['videos'] = [];
  ...
  // Extract watchlist
  const watchlist: YouTubeDigestData['watchlistPriority'] = [];
  ...
  const digestData: YouTubeDigestData = {
    date,
